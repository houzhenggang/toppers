...
static int cmd_sh(int argc, char **argv)
{
	static const char nestchcmd[]   = "\nno nest shell command !\n\015";
	static char shellpath[PATH_LEN+4];
	static FILE save_file;
	static BOOL shcmdmode = FALSE;
	FILE *fd;
	INT ch;

	if(shcmdmode){
		fwrite(nestchcmd, sizeof(nestchcmd), 1, stderr);
		return -1;
	}
	if(argc < 2){
		fwrite(noargment, sizeof(noargment), 1, stderr);
	...
	if(mkpath(argv[1], shellpath) == NULL){
		fwrite(nofile, sizeof(nofile), 1, stderr);
		return -1;
	}
	fd = fopen(shellpath, "rb");
	if(fd == NULL){
		fwrite(nofile, sizeof(nofile), 1, stderr);
		return -1;
	}
	...
	copy_file_func(fd, stdin);
	do{
		ch = dispatcher(1);
	}while(ch != EOF);
	fclose(fd);
	copy_file_func(&save_file, stdin);
	shcmdmode = FALSE;
	printf("end shell command\n");
	return 0;
}
