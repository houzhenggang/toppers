 ...
 * 
 *  @(#) $Id: stdio.c,v 1.2 2010/07/09 18:39:56 roi Exp $
 */
/*
...
void stdfile_init(VP_INT exinf)
{
	unsigned int idx;
...
off_t lseek(int fd, off_t offset, int whence)
{
	...
	if(psdev->pdevff != 0 && psdev->pdevff->_sdevff_lseek != 0)
		return psdev->pdevff->_sdevff_lseek(ps->sdmfd, offset, whence);
	else
		return -1;
}
...
long read(int fd, void *buf, long count)
{
	...
	if(psdev->pdevff != 0 && psdev->pdevff->_sdevff_read != 0)
		return psdev->pdevff->_sdevff_read(ps->sdmfd, buf, count);
	else
		return 0;
}
...
long write(int fd, const void *buf, long count)
{
	...
	if(psdev->pdevff != 0 && psdev->pdevff->_sdevff_write != 0)
		return psdev->pdevff->_sdevff_write(ps->sdmfd, buf, count);
	else
		return 0;
}
...
static int local_getc(void *st)
{
	...
	if((psdev = (StorageDevice_t *)((FILE*)st)->_dev) == 0)
		return -1;
	if(psdev->pdevff != 0 && psdev->pdevff->_sdevff_read != 0){
		result = psdev->pdevff->_sdevff_read(((FILE *)st)->_file, buf, 1);
		if(result == 1)
			return buf[0];
		else
			return -1;
...
static int local_gets(void *st, unsigned int len, char *s)
{
	...
	if((psdev = (StorageDevice_t *)((FILE*)st)->_dev) == 0)
		return -1;
	if(psdev->pdevff != 0 && psdev->pdevff->_sdevff_read != 0)
		return psdev->pdevff->_sdevff_read(((FILE *)st)->_file, s, len);
	else
		return 0;
}
...
static void local_putc(void *st, int c)
{
	...
	buf[0] = c;
	if(psdev->pdevff != 0 && psdev->pdevff->_sdevff_write != 0)
		psdev->pdevff->_sdevff_write(((FILE *)st)->_file, buf, 1);
}
...
static int local_puts(void *st, unsigned int len, char *s)
{
	...
	if((psdev = (StorageDevice_t *)((FILE*)st)->_dev) == 0)
		return -1;
	if(psdev->pdevff != 0 && psdev->pdevff->_sdevff_write != 0)
		return psdev->pdevff->_sdevff_write(((FILE *)st)->_file, s, len);
	else
		return 0;
}
