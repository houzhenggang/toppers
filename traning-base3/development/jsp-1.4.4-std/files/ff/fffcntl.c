 ...
 * 
 *  @(#) $Id: fffcntl.c,v 1.1 2010/07/16 18:39:56 roi Exp $
 */
...
static int ff_open(const char *pathname, int flags);
static int ff_close(int fd);
static int ff_fstat(int fd, struct stat *buf);
static off_t ff_lseek(int fd, off_t offset, int whence);
static long ff_read(int fd, void *buf, long count);
static long ff_write(int fd, const void *buf, long count);
...
static char  sddevname[256] = "0:";
static int   ff_errno;

/*
 *  デバイスの設定
 */
void _sd_setfcntldev(int no)
{
	sddevname[0] = (char)no+'0';
}
