/*
 *  TOPPERS/ASP/JSP Kernel
 *      Toyohashi Open Platform for Embedded Real-Time Systems/
 *      Advanced/Just Standard Profile Kernel
 * 
 ...
 * 
 *  @(#) $Id: stdfile.c,v 1.2 2012/07/01 18:10:56 roi Exp $
 */
/*
 * TOPPERS/JSP-ASP+FATFS対応のPOSIXファイル関数
 */
#include <kernel.h>
#include <stdio.h>
#include <string.h>
...
FILE *fopen(const char *name, const char *type)
{
  ...
  if((fp = alloc_file()) != 0){
    ...
    if(stat == 3)
      ...
      mode = stat;
    id = pff->_sdevff_open(name, mode|O_CREAT);
    if(id < 0){
      free_file(fp);
      return NULL;
    }
...
int fseek(FILE *fp, long offset, int whence)
{
  ...
  if(psdev->pdevff != 0 && psdev->pdevff->_sdevff_lseek != 0)
    return psdev->pdevff->_sdevff_lseek(fp->_file, offset, whence);
  else
    return -1;
}
...
int stat(const char *name, struct stat *buf)
{
  ...
  if(psdev->pdevff == 0 || psdev->pdevff->_sdevff_stat == 0)
    return -1;
  if(psdev->pdevff->_sdevff_stat(name, buf) >= 0)
    return 0;
  else
    return -1;
}
...
int access(const char *path, int mode)
{
  ...
  if(stat(path, &buf) >= 0){
    res = 0;
    if((mode & 0x02) != 0 && (buf.st_flags & 0x001) != 0)	/* read only(no debug) */
      res = -1;
  }
  return res;
}
