 ...
 * 
 *  @(#) $Id: hw_serial.h,v 1.2 2007/10/20 14:52:22 roi Exp $
 */

/*
 *  シリアルI/Oデバイス（SIO）ドライバ（lpc23xx用）
 */
 ...
 */
#define INHNO_SIO0  INTNO_UART0
#define INHNO_SIO1  INTNO_UART1
...
sio_opn_por(ID siopid, VP_INT exinf)
{
    ...
    /* Enable Interrupt Rx */
    if(!openflag){
		sil_wrw_mem((VP)(TADR_VIC_BASE+TOFF_VIC_INTENABLE), 1<<siopcb->siopinib->intno);
    }
    return(siopcb);
}
...
sio_cls_por(SIOPCB *siopcb)
{
    ...
    if (!uart_openflag()) {
		...
		sil_wrw_mem((VP)(TADR_VIC_BASE+TOFF_VIC_INTENCLR), 1<<siopcb->siopinib->intno);
    }
}
