...
sio_opn_por(ID siopid, VP_INT exinf)
{
	...
	*/
    ...
    if ( regBase == UART0_ADDRESS ) {        // UART0の場合
        ...
        *pPORTG_MUX  = (*pPORTG_MUX & ~0x0030) | 0x0020;   // 機能をUART0に割り振る
        *pPORTG_FER |=  0x0180;   // PG7,PG8をUART0に割り振る
    }
    else if ( regBase == UART1_ADDRESS ){    // uart1の場合
        ena_int( INTNO_UART1_TX );
        ena_int( INTNO_UART1_RX );
        *pPORTF_MUX  = (*pPORTF_MUX & ~0x0C00) | 0x0800;   // 機能をUART1に割り振る
        *pPORTF_FER |=  0xC000;   // PF14,PF15をUART1に割り振る
    }
