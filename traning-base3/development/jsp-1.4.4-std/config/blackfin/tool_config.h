...
/*
 *  ラベルの別名を定義するためのマクロ
 *  config.txtによるとこのマクロは定義できないときには定義しなくていい。
 *  VDSP++3.5 for 16bitでは定義できそうにないので止めておく。
 */
...
/*
 *  開発環境依存の初期化は使用しない
 *	VisualDSP++は特に開発環境依存の初期化を必要としない。
 */
#define tool_initialize()

/*
 *  atexit の処理とデストラクタの実行は行わない
 *  VisualDSP++は特に開発環境依存の終了処理を必要としない。
 */
#define call_atexit()
...
/*
 *  __EMPTY_LABEL
 *	VisualDSP++3.5は長さ0の配列宣言を受け付けるが、領域を確保する
 *	JSP 1.4で長さ0の配列を作る理由がわからないので何も定義しないでおく。
 */
