...
/* disable interrupt */
Inline void cpu_disable_int(INTNO intno) {
	set_sr(current_sr() & ~(Cause_IP0 << intno));
}

/* enable interrupt */
Inline void cpu_enable_int(INTNO intno) {
	set_sr(current_sr() | (Cause_IP0 << intno));
}

/* probe interrupt */
Inline BOOL cpu_probe_int(INTNO intno) {
	UW cause;
	Asm(
			"mfc0 %0, $13"
			: "=r"(cause)
	);
	return cause & (Cause_IP0 << intno);
}
