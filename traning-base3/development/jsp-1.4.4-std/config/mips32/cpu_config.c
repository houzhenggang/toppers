 ...
 */
INT_TABLE int_table[ TNUM_ALL_INTNO ];

/*
 *  例外ハンドラの擬似テーブル
 */
FP exc_table[ TNUM_CORE_EXCNO ];
...
void cpu_initialize() {
	...
	for( i = 0; i < TNUM_ALL_INTNO; i++ ) {
		define_inh( i, (FP) &cpu_experr );
	}

	/* 例外ベクタの擬似テーブル初期化 */
	for( i = 0; i < TNUM_CORE_EXCNO; i++ ) {
 ...
 *  dis_dsp を併用すればよい。
 *  MIPS32ターゲットでは、MIPS32コアの割込みマスクだけでなく、割込みコントローラ
...
SYSCALL ER chg_ipm(IPM ipm) {
	...
	t_lock_cpu();
	cpu_set_ipm( ipm.core );	/* MIPS32コアの割込みマスクの設定 */
#if TNUM_ICU_INTNO == 0
...
SYSCALL ER get_ipm(IPM *p_ipm) {
	...
	t_lock_cpu();
	p_ipm->core = cpu_get_ipm();	/* MIPS32コアの割込みマスク参照 */
#if TNUM_ICU_INTNO == 0
#else /* TNUM_ICU_INTNO != 0 */
