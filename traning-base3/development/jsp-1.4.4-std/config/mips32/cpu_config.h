...
extern INT_TABLE int_table[ TNUM_ALL_INTNO ];

/*  例外ハンドラ擬似テーブル  */
extern FP	 exc_table[ TNUM_CORE_EXCNO ];
...
Inline BOOL exc_sense_lock(VP p_excinf) {
	...
	/* Statusレジスタは、スタックに22番目に積まれている。
	   exc_stack(mips32.h), cpu_support.S 参照 */
	return( !( ((EXCSTACK *)p_excinf)->CP0_Status & SR_IE) );
}
...
Inline void all_set_ilv(INTNO intno, IPM *ipm) {
	...
	int_table[intno].intmask = ipm->core;
