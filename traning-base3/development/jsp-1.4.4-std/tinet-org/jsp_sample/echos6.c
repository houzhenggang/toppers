 ...
 * 
 *  @(#) $Id: echos6.c,v 1.5 2009/12/24 05:40:11 abe Exp abe $
 */
...
tcp_echo_srv_task(VP_INT exinf)
{
	...
	while (TRUE) {
		if ((error = tcp6_acp_cep((INT)exinf, TCP_ECHO_SRV_REPID, &dst, TMO_NBLK)) != E_WBLK) {
...
tcp_echo_srv_task(VP_INT exinf)
{
	...
	while (TRUE) {
		if (tcp6_acp_cep((INT)exinf, TCP_ECHO_SRV_REPID, &dst, TMO_FEVR) != E_OK) {
