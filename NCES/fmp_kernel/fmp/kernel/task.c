 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: task.c 716 2010-10-05 06:57:54Z ertl-honda $
 */
 ...
 *  自プロセッサに対して呼び出す
 *
 *  割込み優先度マスクが全解除状態でない場合に，タスク例外処理ルーチンが
 *  起動され，起動されたタスク例外処理ルーチン内で割込み優先度マスクを全
 *  解除状態とした場合には，タスク切り替えが発生して，タスク例外処理ルー
 *  チンを実行しているタスクがマイグレーションされる場合がある．
 *  この場合，タスク例外処理ルーチン終了後，マイグレーション先のプロセッ
 *  サにおいて，タスク例外処理ルーチン開始時の割込み優先度マスクを戻す．
 *  これは，タスク例外処理ルーチンが関連付けられているタスクが例外要求を
 *  受け付けた時点での割込み優先度マスクの値であるため，問題ない．
 *  また，マイグレーション元のプロセッサは，割込み優先度マスクが全解除状
 *  態となっている．
 */
void
