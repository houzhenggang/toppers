...
const CYCINIB _kernel_cycinib_table[TNUM_CYCID] = {
	{ (TA_STA), (intptr_t)(0), (cyc_handler), (ROT_INTERVAL), (0), 1, (uint_t)(0x0000000f) }
};

CYCCB _kernel_cyccb_CYC_HANDLER	__attribute__((section(".bss._kernel_prc1s_objcb"),nocommon));

CYCCB* const _kernel_p_cyccb_table[TNUM_CYCID] = {
	&_kernel_cyccb_CYC_HANDLER
};
...
#error USE_NATIVE_SPIN_LOCK is not defined.
#endif /* TTYPE_SPN != NATIVE_SPN */

const SPNINIB _kernel_spninib_table[TNUM_SPNID] = {
	{(TA_NULL)}
};

SPNCB _kernel_spncb_SPN1 __attribute__((section(".bss._kernel_prc1s_objcb"),nocommon));

SPNCB* const _kernel_p_spncb_table[TNUM_SPNID] = {
	&_kernel_spncb_SPN1
};
