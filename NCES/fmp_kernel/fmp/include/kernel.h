 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: kernel.h 661 2010-07-29 10:10:12Z ertl-honda $
 */

/*
 *		TOPPERS/FMPカーネル 標準ヘッダファイル
 *
 *  TOPPERS/FMPカーネルがサポートするサービスコールの宣言と，必要なデー
...
typedef struct t_rdtq {
	...
	uint_t	sdtqcnt;	/* データキューに入っているデータの数 */
} T_RDTQ;

typedef struct t_rpdq {
	ID		stskid;		/* 優先度データキューの送信待ち行列の先頭タスク
						   のID番号 */
	ID		rtskid;		/* 優先度データキューの受信待ち行列の先頭タスク
						   のID番号 */
	uint_t	spdqcnt;	/* 優先度データキューに入っているデータの数 */
} T_RPDQ;

typedef struct t_rmbx {
	ID		wtskid;		/* メールボックスの待ち行列の先頭のタスクのID番号 */
	T_MSG	*pk_msg;	/* メッセージキューの先頭につながれたのメッセージ */
} T_RMBX;

typedef struct t_rmpf {
	ID		wtskid;		/* 固定長メモリプールの待ち行列の先頭のタスク
						   のID番号 */
	uint_t	fblkcnt;	/* 固定長メモリプールの空きメモリブロック数 */
} T_RMPF;
 ...
 *  タスク管理機能
 */
extern ER		mig_tsk(ID tskid, ID prcid) throw();
extern ER		mig_slf(ID coreid) throw();
extern ER		act_tsk(ID tskid) throw();
extern ER		iact_tsk(ID tskid) throw();
extern ER		mact_tsk(ID tskid, ID prcid) throw();
extern ER		imact_tsk(ID tskid, ID prcid) throw();
...
extern ER		try_spn(ID spnid) throw();
extern ER		itry_spn(ID spnid) throw();
extern ER		ref_spn(ID spnid, T_RSPN *pk_rspn) throw();
...
extern ER		imsta_alm(ID almid, RELTIM almtim, ID prcid) throw();
...
extern ER		mrot_rdq(PRI tskpri, ID prcid) throw();
extern ER		imrot_rdq(PRI tskpri, ID prcid) throw();
extern ER		get_tid(ID *p_tskid) throw();
extern ER		iget_tid(ID *p_tskid) throw();
extern ER		get_pid(ID *p_prcid) throw();
...
#define TA_ENAINT		UINT_C(0x01)	/* 割込み要求禁止フラグをセット*/
...
#define TSPN_UNL		UINT_C(0x01)	/* スピンロックが取得されていない状態 */
#define TSPN_LOC		UINT_C(0x02)	/* スピンロックが取得されている状態 */
...
#define TPRI_SELF		0			/* 自タスクのベース優先度指定 */
#define TPRI_INI		0			/* タスクの起動時優先度指定 */

#define TIPM_ENAALL		0			/* 割込み優先度マスク全解除 */

#define TPRC_INI		0			/* 初期割付けプロセッサ指定 */

#define TPRC_NONE		0			/* 割付けプロセッサの指定がない */
...
#define TOPPERS_SUPPORT_MULTI_PRC	/* マルチプロセッサ対応のカーネル */
 ...
 */
#define G_KLOCK		UINT_C(0x00)
#define P_KLOCK		UINT_C(0x01)
#define F_KLOCK		UINT_C(0x02)

/*
 *  スピンロック機能の方式（TTYPE_SPN）
 */
